...
def TrainCNN():
    ...
    CNN_weights = (
        weights_dir + "DM-CNN_model_{}_epoch_{}_batch_id_{}_labels_{}_step_{}.pwf"
    )
    ...
    train_data = mpid_data_binary.MPID_Dataset(
        train_file, "image2d_image2d_binary_tree", train_device, plane=0, augment=False
    )
    train_loader = DataLoader(
        dataset=train_data, batch_size=cfg.batch_size_train, shuffle=True
    )
    labels = 2

    # Test data
    test_data = mpid_data_binary.MPID_Dataset(
        test_file, "image2d_image2d_binary_tree", train_device, plane=0
    )
    test_loader = DataLoader(
        dataset=test_data, batch_size=cfg.batch_size_test, shuffle=True
    )
    ...
    optimizer = optim.Adam(
        mpid.parameters(), lr=cfg.learning_rate
    )  # , weight_decay=0.001)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (x_batch, y_batch, info_batch, nevents_batch) in enumerate(
            train_loader
        ):
            print("\n")
            ...
            if batch_idx % cfg.test_every_step == 1 and cfg.run_test:
                if cfg.save_weights and epoch >= 3 and epoch <= 6:
                    torch.save(
                        mpid.state_dict(),
                        CNN_weights.format(timestr(), epoch, batch_idx, labels, step),
                    )
                ...
                print(
                    "Start eval on training sample...@epoch..{}.@batch..{}".format(
                        epoch, batch_idx
                    )
                )
