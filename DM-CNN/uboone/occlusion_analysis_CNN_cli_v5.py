...
def run_one(
    ...
    meta_path = out_prefix.with_name(out_prefix.name + f"_meta_{occlusion_size}.txt")
    meta_path.write_text(json.dumps(meta, indent=2))
    print("[ok]", meta_path)

    if save_npy:
        np.save(out_prefix.with_name(out_prefix.name + "_input.npy"), original_img)
        np.save(out_prefix.with_name(out_prefix.name + f"_signal_map_{occlusion_size}.npy"), sig_map_raw)
        np.save(out_prefix.with_name(out_prefix.name + f"_background_map_{occlusion_size}.npy"), bkg_map_raw)
