...
def PrintImage(ENTRY):
    ...
    img = ax.imshow(
        input_image.cpu()[0][0].detach().cpu().numpy().transpose(),
        origin="lower",
        cmap="jet",
        norm=colors.PowerNorm(
            gamma=0.35,
            vmin=input_image.cpu()[0][0].min(),
            vmax=input_image.cpu()[0][0].max(),
        ),
    )
    ...
    if colorbar_included:
        ...
        cbar.set_ticks([0.0, np.max(input_image.cpu()[0][0].detach().cpu().numpy())])
        cbar.ax.set_yticklabels(
            ["Low charge", "High charge"], color="white", fontsize=50
        )
        newax_cb.axis("off")
    ...
    if logo:
        ...
        im = plt.imread(
            logo_path + "kalekologo_noshadow_notrack_inverted.png"
    ...
    if data:
        ...
        ax.text(
            20,
            20,
            "Run: {}, Subrun: {}, Event {}".format(
                event_info[0], event_info[1], event_info[2]
            ),
            color="white",
            fontsize=53,
        )
