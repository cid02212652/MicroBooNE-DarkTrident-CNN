...
def score_plot(score_map, output_dir, tag, title, vmin, vmax, cmap_input="gnuplot_r"):
    ...
    pos = ax.imshow(
        score_map.transpose(), origin="lower", cmap=cmap_input, vmin=vmin, vmax=vmax
    )
    ...
    plt.savefig(
        output_dir + "occlusion_test_{}_{}_map.png".format(tag, title),
        bbox_inches="tight",
    )
    plt.savefig(
        output_dir + "occlusion_test_{}_{}_map.pdf".format(tag, title),
        bbox_inches="tight",
    )
...
def RunOcclusion(input_entry):
    ...
    DataLoader(dataset=test_data, batch_size=1, shuffle=True)
    ...
    for ENTRY in range(entry_start, entry_start + entries):
        ...
        score_map_signal = np.full(
            [512 - occlusion_step, 512 - occlusion_step],
            score.cpu().detach().numpy()[0][0],
        )
        score_map_background = np.full(
            [512 - occlusion_step, 512 - occlusion_step],
            score.cpu().detach().numpy()[0][1],
        )
        ...
        for x in range(0 + occlusion_step, 512 - occlusion_step):
            for y in range(0 + occlusion_step, 512 - occlusion_step):
                ...
                clone_image[0][0][
                    x - occlusion_step : x + occlusion_step + 1,
                    y - occlusion_step : y + occlusion_step + 1,
                ] = torch.zeros([2 * occlusion_step + 1, 2 * occlusion_step + 1])
    ...
    if normalized:
        ...
        score_map_background_norm = (score_map_background - vmin_background) / (
            delta_background
        )

        vmin_signal_final = 0.0
        vmax_signal_final = 1.0

        vmin_background_final = 0.0
    ...
    score_plot(
        score_map_signal_norm,
        output_dir,
        image_tag,
        "Signal",
        vmin_signal_final,
        vmax_signal_final,
        cmap_input="gnuplot_r",
    )
    score_plot(
        score_map_background_norm,
        output_dir,
        image_tag,
        "Background",
        vmin_background_final,
        vmax_background_final,
    )
