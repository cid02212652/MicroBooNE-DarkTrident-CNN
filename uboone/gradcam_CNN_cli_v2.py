...
def _deletion_insertion_curves(
    ...
    """Faithfulness curves. Uses sigmoid(logit[idx]) for each class.

    deletion: set top-k% pixels -> baseline
    insertion: start at baseline and insert top-k% pixels
    also computes one random baseline curve (same k values).

    Returns dict with curves + simple AUCs.
    """
    import numpy as _np
    ...
    H, W = attr.shape
    flat = (np.abs(attr) if use_abs_rank else attr).reshape(-1)
    n = flat.size
    ...
    fracs = _np.linspace(0.0, 1.0, int(n_steps))
    out = {
    ...
    # Pre-allocate a flat boolean mask to avoid tons of allocations
    mask_flat = _np.zeros(n, dtype=bool)
    rmask_flat = _np.zeros(n, dtype=bool)
    ...
    # AUC summaries
    out["auc"] = {
        "deletion_signal": _auc(out["deletion"]["signal"], fracs),
        "deletion_background": _auc(out["deletion"]["background"], fracs),
        "insertion_signal": _auc(out["insertion"]["signal"], fracs),
        "insertion_background": _auc(out["insertion"]["background"], fracs),
...
def _preset_layer_name(model: nn.Module, preset: str) -> str:
    """Return a sensible layer name for CAM hooks.

    Presets are designed around matching *spatial grid size* across MPID vs ResNet.
      - mid128: ResNet layer1, MPID features.7
      - mid64:  ResNet layer2, MPID features.14
      - mid32:  ResNet layer3, MPID features.21
      - final:  ResNet layer4, MPID features.31 (final conv before pooling)
...
def run_one(
    ...
    if layer_name is None:
        layer_name = default_cam_layer_name(model, preset=layer_preset)
    ...
    if diag_curves:
        try:
            curves = _deletion_insertion_curves(
                model=model,
                x=x,
                attr=sig_map,
                signal_idx=0,
